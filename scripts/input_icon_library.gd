...
func _init():
	if keyboard_icons_path == "": keyboard_icons_path = InputConfig.icon_keyboard_path
	if mouse_icons_path == "": mouse_icons_path = InputConfig.icon_mouse_path
	if gamepad_icons_path == "": gamepad_icons_path = InputConfig.icon_gamepad_path
...
func _get_automatic_path(event: InputEvent) -> String:
	...
	if event is InputEventKey: folder = keyboard_icons_path
	elif event is InputEventMouseButton: folder = mouse_icons_path
	elif event is InputEventJoypadButton or event is InputEventJoypadMotion: folder = gamepad_icons_path
	if folder.is_empty() or file_name.is_empty(): return ""
